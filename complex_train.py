#!/usr/bin/env python3
"""Training script for complex tensor decomposition."""
...
def train(config):
    """Train the tensor decomposition model."""
    # Set random seed
    ...
    # Data
    dataset = Rank1ComplexTensorDataset(config, config.batch_size)
    loader = iter(dataset)

    # Model
    model = ComplexTensorDecompositionMLP(config)
    model = model.to(config.device)
    layer_type = "Widely Linear" if config.use_widely_linear else "Strictly Linear"
    print(f"Using {layer_type} layers")
    ...
    print("Using full precision training with complex numbers")
    ...
    for batch_idx in range(config.n_train_batches):
        ...
        if (batch_idx + 1) % config.eval_interval == 0:
            ...
            pbar.write(f"U(2) Equivariance - Mean variance: {eval_results.get('equivariance/mean_variance', 0):.5f}, Max variance: {eval_results.get('equivariance/max_variance', 0):.5f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train complex tensor decomposition model')
    parser.add_argument('--config', type=str, default='configs/complex_default.yaml',
