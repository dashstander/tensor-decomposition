"""Evaluation functions for real tensor decomposition models."""
...
def evaluate_comprehensive(model, config, n_test_batches=20):
    ...
    with torch.no_grad():
        for batch_idx in range(n_test_batches):
            ...
            dataset = Rank1RealTensorDataset(config, batch_size)
            ...
            for i, (pred, true) in enumerate(zip(factors_pred, factors_true)):
                cosine_sim = (pred * true).sum(dim=-1)
                per_factor_loss = 1 - torch.abs(cosine_sim)  # Per-sample losses
            ...
            for tensor_type in [1, 2, 3]:
                ...
                if mask.any():
                    ...
                    for pred, true in zip(factors_pred, factors_true):
                        cosine_sim = (pred[mask] * true[mask]).sum(dim=-1)
                        type_factor_loss += torch.mean(1 - torch.abs(cosine_sim))
...
def evaluate_equivariance(model, config, metrics, results):
    """
    Test SO(n) equivariance of the model.
    ...
    with torch.no_grad():
        for sample_idx in range(n_equivariance_samples):
            ...
            for pred_factor, true_factor in zip(factors_pred_batch, factors_true_batch):
                ...
                factor_losses = 1 - torch.abs(cosine_sim)  # Per-sample losses
